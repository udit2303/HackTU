...
def get_user_areas(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    return (
        db.query(Area).filter(Area.user_id == user_id).offset(skip).limit(limit).all()
    )
...
def get_measurements_by_area(
    db: Session, area_id: int, skip: int = 0, limit: int = 100
):
    return (
        db.query(SoilMeasurement)
        .filter(SoilMeasurement.area_id == area_id)
