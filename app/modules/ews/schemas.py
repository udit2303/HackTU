...
class ScenarioInput(BaseModel):
    ...
    total_mm: Optional[float] = Field(None, description="Total rainfall in mm (for RAINFALL scenarios)")
    duration_hours: Optional[int] = Field(None, description="Duration in hours (for RAINFALL scenarios)")
    rise_meters: Optional[float] = Field(None, description="Groundwater rise in meters (for GROUNDWATER_RISE scenarios)")
...
class MetricsOutput(BaseModel):
    ...
    soil_moisture_avg: float = Field(..., description="Average soil moisture (0.0 to 1.0)")
    displacement_trend_mm_per_month: float = Field(..., description="Displacement trend in mm/month")
...
class HeatmapFeatureProperties(BaseModel):
    ...
    risk_score: float = Field(..., description="Risk score for this cell (0.0 to 1.0)", ge=0.0, le=1.0)
    risk_level: RiskLevel = Field(..., description="Risk level classification")
...
class EvacuationRoute(BaseModel):
    ...
    risk_score: float = Field(..., description="Risk score for this route (0.0 to 1.0)", ge=0.0, le=1.0)
    eta_minutes: int = Field(..., description="Estimated time of arrival in minutes")
    geometry: GeoJSONGeometry = Field(..., description="LineString geometry of the route")
...
class ScenarioRequest(BaseModel):
    ...
    scenarios: List[ScenarioInput] = Field(..., description="List of scenarios to simulate")
...
class EvacuationRequest(BaseModel):
    ...
    start_points: List[Coordinate] = Field(..., description="Starting points for evacuation")
...
class AnalyticsResponse(BaseModel):
    ...
    stability_index: float = Field(..., description="Ground stability index (0.0 to 1.0)", ge=0.0, le=1.0)
    confidence: float = Field(..., description="Confidence in the analysis (0.0 to 1.0)", ge=0.0, le=1.0)
...
class PredictionResponse(BaseModel):
    ...
    risk_score: float = Field(..., description="Risk score (0.0 to 1.0)", ge=0.0, le=1.0)
    risk_level: RiskLevel = Field(..., description="Risk level classification")
    confidence: float = Field(..., description="Confidence in prediction (0.0 to 1.0)", ge=0.0, le=1.0)
    expected_window: Optional[ExpectedWindow] = Field(None, description="Expected time window for event")


class ScenarioResponse(BaseModel):
    """Response from scenario simulation."""
    baseline_risk: float = Field(..., description="Baseline risk score (0.0 to 1.0)", ge=0.0, le=1.0)
...
class PredictionHistoryItem(BaseModel):
    ...
    expected_window_from: Optional[datetime] = Field(None, description="Expected window start")
    expected_window_to: Optional[datetime] = Field(None, description="Expected window end")
